...
data1$age <- dvage^2/50
data1$sex <- as.factor(ifelse(sex<0,NA,sex))
data1$fimngrs_dv <- ifelse(data1$fimngrs_dv==c(-9,-8,-2,-1),NA,data1$fimngrs_dv)
summary(data1$edu3)
summary(data1$sclfsato)
reg01 <- lm(sclfsato ~ sex + dvage + age + edu3 + region4, data = data1)
data1 <- na.omit(data1)
...
data1$relrk <- 1 - (data1$absrk/data1$grpsize)
write_dta(data1,"rrankm1.dta")
```
```{r}
  data1$agegrp <- ifelse((data1$dvage < data1$dvage[1]+6.5 & data1$dvage > data1$dvage[1]-3.5),1,0) 
  data1$refg <- data1$agegrp*as.numeric(data1$group_m1)
  data1$refg <- ifelse(data1$refg == data1$refg[1],1,NA)
  data1$inc <- data1$refg*data1$fimngrs_dv
  data1$absrk[1] <- sum(!is.na(data1$inc))-rank(data1$inc)[1] 
```
