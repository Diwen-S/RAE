...
data2 <- subset(data2,select = !(names(data2) %in% c("pidp","hidp","scend","edu3","gor_dv","refg","inc","agegrp","groupm1")))
...
data2$grpsize1<-ifelse(data2$grpsize>1,data2$grpsize,NA)
data2$relrk1 <- (data2$absrk1-1)/(data2$grpsize1-1)
save(data2,file = "use_reg2.RData")
load("use_reg2.RData")
write_dta(data2,"use_reg2.dta")
